...
const TarotReading = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const selectedCardIds = selectedCards.join(',');
      ...
      setDisplayedCards(interpretedCards);
            ...
            <div className="result-container">
              {displayedCards.map((card, index) => (
                <div key={index} className="card-result">
                  ...
                  <p className="card-meaning">{card?.meanings?.[card.isReversed ? 'reversed' : 'upright']?.general || 'No meaning available'}</p>
                </div>
              ))}
            </div>
